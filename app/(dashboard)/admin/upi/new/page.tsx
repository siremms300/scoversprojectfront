...
export default function NewUPIProgramPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = {
        ...
        transferableTo: form.transferableTo.filter((t: any) => t.institution.trim())
      }
      ...
      toast.success('UPI Program created successfully!')
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/admin/upi" className="p-2 rounded-xl hover:bg-gray-100 transition-colors">
          <ArrowLeft className="w-5 h-5 text-gray-500" />
        </Link>
        <div>
          <h1 className="text-[22px] font-bold text-gray-900">Create UPI Program</h1>
          <p className="text-[13px] text-gray-500 mt-0.5">Add a new university pathway program</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Requirements */}
        <div className="bg-white rounded-[20px] border border-gray-100/80 p-6">
          <h2 className="text-[16px] font-semibold text-gray-900 mb-5">Requirements</h2>
          <div className="mb-4">
            <label className="block text-[13px] font-medium text-gray-700 mb-3">Entry Requirements</label>
            ...
            <button type="button" onClick={() => addArrayItem('requirements')} className="flex items-center gap-2 text-[13px] text-[#247BF7] hover:underline mt-2">
              <Plus className="w-4 h-4" /> Add Requirement
            </button>
          </div>

          <div>
            <label className="block text-[13px] font-medium text-gray-700 mb-3">Start Dates</label>
            ...
            <button type="button" onClick={() => addArrayItem('startDates', '')} className="flex items-center gap-2 text-[13px] text-[#247BF7] hover:underline mt-2">
              <Plus className="w-4 h-4" /> Add Start Date
            </button>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3">
          <Link href="/admin/upi" className="px-6 py-3 border border-gray-200 rounded-2xl text-[13px] font-medium text-gray-600 hover:bg-gray-50 transition-colors">
            Cancel
          </Link>
