...
import {
  Search, Eye, ChevronRight, Filter,
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-gray-500 mt-0.5">View all applications and their status</p>
        </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                <tr key={app._id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <span className="text-[13px] text-gray-700">{app.programType === 'UPI' ? 'UPI' : app.programType}</span>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold ${statusColors[app.status] || 'bg-gray-100 text-gray-600'}`}>
                      {app.status?.replace(/_/g, ' ')}
                    </span>
                  </td>
                  <td className="px-5 py-3.5">
                    {app.probabilityScore ? (
                      <span className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold ${probabilityColors[app.probabilityScore] || ''}`}>
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <Link
                      href={`/investor/applications/${app._id}`}
