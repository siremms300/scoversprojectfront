...
import {
  ...
  CreditCard, TrendingUp, ArrowRight
} from 'lucide-react'
...
export default function UserApplicationDetailPage() {
  ...
  if (!application) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/dashboard/applications" className="text-[#247BF7] hover:underline mt-2 inline-block">
  ...
  return (
    <div className="space-y-6">
      <Link href="/dashboard/applications" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Sidebar */}
        <div className="space-y-6">
          {/* Enrollment Status for UPI */}
          {application.programType === 'UPI' && application.status === 'completed' && (
            <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-[20px] border border-emerald-200 p-5">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="font-semibold text-emerald-800">You're Enrolled!</p>
                  <p className="text-[12px] text-emerald-600">Access your learning materials now</p>
                </div>
              </div>
              <Link
                href="/dashboard/upi"
                className="flex items-center justify-center gap-2 w-full py-2.5 bg-emerald-500 text-white rounded-xl text-[13px] font-medium hover:bg-emerald-600 transition-colors"
              >
          ...
          {application.programType === 'UPI' && application.status === 'accepted' && (
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-[20px] border border-blue-200 p-5">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-blue-100 rounded-xl flex items-center justify-center">
                  <Clock className="w-5 h-5 text-blue-600" />
                </div>
                <div>
                  <p className="font-semibold text-blue-800">Application Accepted!</p>
                  <p className="text-[12px] text-blue-600">Waiting for enrollment confirmation. You'll be notified soon.</p>
                </div>
              </div>
            </div>
          )}

          {application.programType === 'UPI' && application.status === 'rejected' && (
            <div className="bg-gradient-to-br from-red-50 to-orange-50 rounded-[20px] border border-red-200 p-5">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-red-100 rounded-xl flex items-center justify-center">
                  <XCircle className="w-5 h-5 text-red-600" />
                </div>
                <div>
                  <p className="font-semibold text-red-800">Application Not Accepted</p>
                  <p className="text-[12px] text-red-600">You can apply for other programs</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="bg-white rounded-[20px] border border-gray-100/80 p-5">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: 'Last Updated', value: new Date(application.updatedAt).toLocaleDateString() },
